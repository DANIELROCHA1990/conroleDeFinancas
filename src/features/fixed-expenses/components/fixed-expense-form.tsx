  ...
  return (
    <form action={saveFixedExpenseAction} className="grid gap-3 rounded-[1.25rem] border border-white/10 bg-white/5 p-4">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <input name="default_amount" type="number" step="0.01" min="0.01" defaultValue={expense?.default_amount} placeholder={expense?.amount_mode === "variable" ? "Valor estimado mensal" : "Valor mensal"} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3" required />
